...
const validateSignup = ({ firstName, lastName, email, password, confirmPassword }) => {
  const validateError = {};
  if (firstName === '') {
...
const SignupForm = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const validatedErrors = validateSignup({ ...signupInputs, confirmPassword });
  ...
  const UserTypeSelect = () => (
    <div>
      ...
      <div className="radio-flex">
        <UserTypeRadio title="Job Seeker" imgSrc={seekerIcon} onClick={() => setUserType('seeker')} className={userType === 'seeker' ? 'selected' : ''} />
        {/* <UserTypeRadio disabled
        title="Job Provider"
        imgSrc={providerIcon} onClick={() => setUserType('provider')}
        className={userType === 'provider' ? 'selected' : ''} /> */}
      </div>
      {userType && <div className="next-button-container">
        <Button className="next-button" onClick={() => setUserTypeSelected(true)}>NEXT</Button>
      </div>
      }
    </div>
  );
        ...
        <Form className="registration-details">
          { !userTypeSelected
            ...
            : <>
              ...
              <div className="login-link">Already have an account? <Link to="/user/signin">Sign In</Link></div>
            </>
          }
        </Form>
      </StyledBackground>
